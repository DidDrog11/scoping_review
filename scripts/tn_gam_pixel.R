...
if(!file.exists(here("data_clean", "pop_tn_analysis.rds"))) {
  ...
  pixel_trapping <- st_as_sf(trapping_effort) %>%
    ...
    mutate(hab = factor(case_when(hab == 1 ~ "agriculture",
           ...
           y = st_coordinates(geometry)[,2]) %>%
    select(tn_density, pop, hab, x, y)

  pixel_non_trapped <- tibble(spatSample(resample_hab, size = nrow(pixel_trapping), method = "random", na.rm = TRUE, cells = TRUE, xy = TRUE)) %>%
    mutate(hab = factor(case_when(lccs_class == 1 ~ "agriculture",
                                  ...
                                  lccs_class == 9 ~ "water")),
           tn_density = 0) %>%
    select(tn_density, hab, x, y)

  pixel_non_trapped$pop <- extract(wa_pop, vect(pixel_non_trapped, geom = c("x", "y")))$pop_2005

  pixel_trapping_combined <- bind_rows(pixel_trapping, pixel_non_trapped)

  tn_pixel_simple <- gam(tn_density ~ s(x, y, k = 540),
                         family = "tw",
                         data = pixel_trapping_combined)

  tn_pixel_pop_model <- gam(tn_density ~ s(pop, k = 9) + s(x, y, k = 540),
                            family = "tw",
                            data = pixel_trapping_combined)

  tn_pixel_habitat_model <- gam(tn_density ~ hab + s(x, y, k = 540),
                                family = "tw",
                                data = pixel_trapping_combined)

  tn_pixel_pop_hab_model <- gam(tn_density ~ s(pop, k = 9, by = hab) + s(x, y, k = 540),
                                family = "tw",
                                data = pixel_trapping_combined)
  ...
  model_1_pixel <- getViz(tn_pixel_pop_hab_model)

  fig_2_pixel <- plot(sm(model_1_pixel, 10), n = 150, too.far = 0.02) +
